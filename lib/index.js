...
const Config = Schema.object({
	...
	reviewer: Schema.object({
		mode: Schema.union(REVIEWER_MODES).default("direct").description("How the reviewer runs: `subagent` starts a read-only child that can inspect the workspace; `direct` makes one plain model call with the evidence packet only."),
...
var ReviewRuntime = class {
	...
	*/
	async review(req, session, policySource, rawArguments, override, next) {
		...
		if (this.cacheUsable && override === void 0) {
		...
		else if (this.cacheUsable && override === void 0) this.cache.put(fingerprint, result.verdict);
