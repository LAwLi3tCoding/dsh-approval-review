...
const Config = Schema.object({
	...
	context: Schema.object({
		...
		includeAuthorizations: Schema.boolean().default(true).description("Include the harness's authorization ledger in the reviewer evidence: the user's explicit `ask_user_question` selections, the user's own instructions, and a one-shot `/approve`. NOT bounded by `turns` — an authorization is a durable fact, not recent chatter, and a transcript window is what aged one out of the packet."),
		authorizationMaxChars: Schema.number().step(1).min(0).default(2e3).description("Character budget for the authorization-ledger section; 0 disables it."),
		authorizationMaxEntries: Schema.number().step(1).min(1).default(8).description("Newest authorization facts kept in that section; older ones are dropped, never summarized, because a summarized authorization is one the reviewer would have to guess at.")
	}).default({}),
...
*/
const MECHANISM_VS_ACTION_RULE = `Grade the action, not the mechanism. When the thing under review IS a change to
...
*/
const AUTHORIZATION_TRUST_RULE = `One section of the evidence may be headed
...
function buildReviewerSystemPrompt(config) {
	...
	return `${policy}${config.guidance !== void 0 && config.guidance.trim().length > 0 ? `\n\nDeployment-specific guidance:\n${config.guidance}` : ""}${policy.includes(MECHANISM_VS_ACTION_RULE) ? "" : `\n\n${MECHANISM_VS_ACTION_RULE}`}
...
function buildReviewerUserMessage(evidence) {
	const sections = [];
	if (evidence.authorizations !== void 0 && evidence.authorizations.trim().length > 0) sections.push(`Authorizations recorded by the harness (newest first, may be elided):\n${evidence.authorizations}`);
...
async function runReviewerCall(ctx, route, system, message, limits) {
	...
	try {
		...
		const text = blocksToText$2(assembler.blocks());
...
*/
function blocksToText$2(blocks) {
...
*/
function buildAuthorizationLedger(session, limits) {
	...
	for (let seq = 0; seq < session.seq; seq += 1) {
		...
		switch (event.type) {
			...
			case "user/message": {
				...
				const text = clampText(blocksToText$1(event.data.content), perEntry);
			...
			case "tool/result": {
				...
				if (callId === void 0 || callsByName.get(callId) !== "ask_user_question") break;
				const text = clampText(blocksToText$1(event.data.message.content), perEntry);
				if (text.length === 0) break;
				entries.push({
					at: event.time,
...
*/
function renderAuthorizationLedger(entries, maxChars) {
	...
	if (kept.length === 0) kept.push(clampText(authorizationLine(entries[0]), maxChars));
	const omitted = entries.length - kept.length;
	return `${omitted > 0 ? `[${omitted} older authorization(s) omitted for budget]\n` : ""}${kept.join("\n")}`;
}
/**
...
*/
function overrideEntry(override, turn) {
	return {
		at: override.at,
		turn,
		kind: "override",
		text: clampText(`the human ran /approval-review approve, authorizing one "${override.toolName}" call that had been refused`, OVERRIDE_TEXT_MAX)
	};
}
/** Render one entry as a single ledger line. */
function authorizationLine(entry) {
...
*/
function isHumanAuthored(source) {
	const kind = source?.kind;
	return kind === void 0 || kind === "user";
}
/**
...
function toolResultCallId(event) {
	...
	for (const block of content) if (block.type === "tool-result") return String(block.toolCallId);
}
/**
...
var VerdictCache = class {
	...
	*/
	static fingerprint(toolName, argumentsText, authorizationDigest = "") {
...
async function runSubagentReviewer(ctx, input) {
	...
	const evidence = buildReviewerUserMessage({
		...
		...input.evidence.authorizations === void 0 ? {} : { authorizations: input.evidence.authorizations },
...
var ReviewRuntime = class {
	...
	*/
	async review(req, session, policySource, rawArguments, override, next) {
		...
		const fingerprint = VerdictCache.fingerprint(req.toolName, rawArguments, override === void 0 ? "" : `${override.toolName}#${override.reviewId ?? "latest"}`);
		if (this.cacheUsable) {
	...
	*/
	...
	turnOf(session) {
		let turn = 0;
		for (let seq = 0; seq < session.seq; seq += 1) if (session.eventAt(seq)?.type === "turn/start") turn += 1;
		return turn;
	}
};
/**
