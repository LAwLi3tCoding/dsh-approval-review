...
describe('ReviewSessions overrides', () => {
  it('consumes the newest matching authorization for a tool', () => {
    ...
    const target = session()
    const guard = limits()
    sessions.addOverride(target, { toolName: 'bash', at: Date.now(), reviewId: 'r1' }, guard)
    sessions.addOverride(target, { toolName: 'bash', at: Date.now(), reviewId: 'r2' }, guard)
    expect(sessions.consumeOverride(target, 'bash', guard)?.reviewId).toBe('r2')
    expect(sessions.consumeOverride(target, 'bash', guard)?.reviewId).toBe('r1')
    expect(sessions.consumeOverride(target, 'bash', guard)).toBeUndefined()
  })
  ...
  it('leaves an authorization for another tool alone', () => {
    ...
    const target = session()
    const guard = limits()
    sessions.addOverride(target, { toolName: 'write', at: Date.now() }, guard)
    expect(sessions.consumeOverride(target, 'bash', guard)).toBeUndefined()
  ...
  it('expires an authorization past its TTL', () => {
    ...
    sessions.addOverride(target, { toolName: 'bash', at: Date.now() - 1000 }, guard)
  ...
  it('keeps an authorization forever at a zero TTL', () => {
    ...
    sessions.addOverride(target, { toolName: 'bash', at: 0 }, guard)
    expect(sessions.liveOverrides(target, guard)).toHaveLength(1)
  })
  ...
  it('bounds the pending list', () => {
    ...
    for (let index = 0; index < 5; index += 1) {
      sessions.addOverride(target, { toolName: 'bash', at: Date.now(), reviewId: `r${index}` }, guard)
    }
  ...
  it('drops expired entries when a new one is recorded', () => {
    ...
    const guard = limits({ overrideTtlMs: 10 })
    sessions.addOverride(target, { toolName: 'bash', at: Date.now() - 1000 }, guard)
    sessions.addOverride(target, { toolName: 'bash', at: Date.now() }, guard)
    expect(sessions.liveOverrides(target, guard)).toHaveLength(1)
  })
})
