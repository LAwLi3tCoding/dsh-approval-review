...
| **访问模式第四项** | 在 仅可查看 / 工作区内修改 / 完全权限 旁边多一个「替我审批」。它和「工作区内修改」共享同一套沙箱与审批 knobs，差别只在**谁来裁决**——所以菜单项本身就是开关，选中它插件才接管。靠 `PermissionPresetService.derive()` 先认记录选中项这一点，两者可并存且保持选中。 |
